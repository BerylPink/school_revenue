...
@section('title', 'Human Resources List')
@section('content')
      ...
      <div class="row">
        <div class="col-xl-8 col-lg-8 col-md-8 col-sm-8">
          ...
          <div class="page-title">
            <h5>Human Resources List</h5>
            <h6 class="sub-heading">All Human Resources as of <strong><?php echo date('M, d Y'); ?></h6>
          </div>
        </div>
        ...
        <table id="basicExample" class="table table-bordered table-responsive">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $hr->email }}</td>
              <td>{{ $hr->StateName }}</td>
              <td><?php $date = \Carbon\Carbon::parse($hr->created_at , 'UTC'); echo $date->isoFormat('MMMM Do YYYY h:mm:ssa'); ?></td>
              <td>
                <div class="dropdown text-center">
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 38px, 0px); top: 0px; left: 0px; will-change: transform;">
                    <a class="dropdown-item" href="{{ route('human-resource.show', ['human-resource' => $hr->id]) }}" title="More details on {{ $hr->firstname.' '.$hr->lastname }}">
                      <span class="icon-profile text-primary"></span> 
                      Details
                    </a>

                      <a class="dropdown-item" href="{{ route('human-resource.edit', ['human-resource' => $hr->id]) }}" title="Edit {{ $hr->firstname.' '.$hr->lastname }}">
                        <span class="icon-edit text-warning"></span> 
                        Edit
                      </a>
                      <form method="POST" action="{{ route('human-resource.destroy', ['human-resource' => $hr->id]) }}">
                        @csrf @method('DELETE')
                        <button type="submit" class="dropdown-item" href=""   title="Delete {{ $hr->firstname.' '.$hr->lastname }}">
